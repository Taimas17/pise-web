...
export default function ChantierDetail(){
  ...
  return (
    <div className="grid gap-4 animate-fade-in">
      ...
      <PageHeader
        title={`Chantier #${chantier.id} — ${chantier.title || chantier.name || ''}`}
        actions={[
          <div key="status" className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground hidden sm:inline">{chantier.progress_pct ?? chantier.progress ?? 0}%</span>
          </div>
        ]}
      />

      <Progress value={Number(chantier.progress_pct ?? chantier.progress ?? 0)} />
